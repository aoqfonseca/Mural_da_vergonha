...
describe Phrases do
  ...
  describe ".initialize" do
    ...
    it "should load phrases" do
      YAML.stub!(:load_file).with(any_args()).and_return(['phrase'])
      phrases = Phrases.new
  ...
  describe ".candidates" do
    ...
    it "should return all the phrases except the ones that have been drawn already" do
      YAML.stub!(:load_file).with(any_args()).and_return(['phrase', 'another phrase'])
      phrases = Phrases.new
  ...
  describe ".select" do
    ...
    before do
      YAML.stub!(:load_file).with(any_args()).and_return([1, 2, 3, 4, 5, 6])
    end
